...
use crate::{identifier::Identifier, bag::{ValueBag, ValueBagTransfer}, value::Value, env::{Environment, EvalError}, query::{UpdateQuery, DeletionQuery, Predicate, ProjectionQuery, TransferQuery, Insertion}, bag::{InsertionResult, UpdateResult}, expression::Expression};
...
impl<'b, 'i, 's, 'v> Transaction<'b, 'i, 's, 'v> {
    ...
    fn get_working_copy_mut(&mut self) -> Result<&mut Cow<'b, BagBundle<'b, 'i, 's, 'v>>, TransactionError> {
        match self {
            Transaction::Clean { working_copy } => Ok(working_copy),
            Transaction::Failed => Err(TransactionError::Aborted),
        }
    ...
    pub(crate) fn insert<'e>(&mut self, 
        bag_name: &Identifier<'s>, 
        env: &'e Environment<'i, 's, 'v>,
        insertion: &Insertion<'s>) -> Result<InsertionResult, TransactionError> {
    ...
    pub(crate) fn update<'e>(&mut self, 
        bag: &Identifier<'s>, 
        env: &'e Environment<'i, 's, 'v>,
        update: &'e UpdateQuery<'s>) -> Result<UpdateResult, TransactionError>  {
    ...
    pub(crate) fn delete<'e>(&mut self, 
        bag: &Identifier<'s>, 
        env: &'e Environment<'i, 's, 'v>,
        deletion: &'e DeletionQuery<'s>) -> Result<DeletionResult, TransactionError> {
    ...
    pub(crate) fn create_bag(&mut self, bag_name: Identifier<'s>, predicate: Predicate<'s>) -> Result<bool, TransactionError> {
        let working_copy = self.get_working_copy_mut()?;
        dbg!(&bag_name);

        if let std::collections::hash_map::Entry::Vacant(e) = working_copy.to_mut().bags.entry(bag_name) {
    ...
    pub(crate) fn get_bag_info(&mut self, bag_name: &Identifier<'s>) -> Result<(usize, &Predicate), TransactionError> {
        let working_copy = self.get_working_copy()?;
    ...
    pub(crate) fn read<'x>(&'x self, bag_name: &'x Identifier) -> Result<impl Iterator<Item = &Cow<'v, Value<'s, 'v>>>, TransactionError> {
        let working_copy = self.get_working_copy()?;
    ...
    pub(crate) fn transfer<'e>(&mut self, source: &Identifier<'s>, sink: &Identifier<'s>, 
        env: &'e Environment<'i, 's, 'v>,
        query: TransferQuery<'s>) -> Result<TransferResult, TransactionError> {
    ...
    pub(crate) fn pop<'x>(&mut self, bag_name: &Identifier<'s>, value: &'x Value<'s,'v>) -> Result<bool, TransactionError> {
        let working_copy = self.get_working_copy_mut()?;
    ...
    pub(crate) fn insert_one<'e,'x:'e>(&mut self, bag_name: &Identifier<'s>, env: &'e Environment<'i, 's, 'v>, expr: &'x Expression<'s>) -> Result<InsertionResult, TransactionError> {
        let working_copy = self.get_working_copy_mut()?;
